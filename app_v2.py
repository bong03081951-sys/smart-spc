...
def overall_verdict(Cpk, nr_violations, nr_viol_count):
    ...
    if cap_good and spc_ok:
        return ("stable", "🟢 안정",
                f"Cpk={Cpk:.4f}(≥1.33) → 공정능력 충분 | Nelson Rule 위반 없음 → 관리 상태")
    elif cap_ok and spc_ok:
        return ("caution", "🟡 주의",
                f"Cpk={Cpk:.4f}(1.00~1.33) → 공정능력 여유 부족 | 관리 상태는 유지 중 → 지속 모니터링 필요")
    elif cap_good and not spc_ok:
        return ("caution", "🟡 주의",
                f"Cpk={Cpk:.4f}(≥1.33) → 공정능력은 충분 | Nelson Rule {nr_viol_count}건 위반 → 이상 원인 조사 필요")
    elif not cap_ok and not spc_ok:
        return ("danger", "🔴 위험",
                f"Cpk={Cpk:.4f}(<1.00) → 불량 발생 가능 | Nelson Rule {nr_viol_count}건 위반 → 두 문제 동시 발생, 즉시 조치 필요")
    else:
        return ("danger", "🔴 위험",
                f"Cpk={Cpk:.4f}(<1.00) → 규격을 벗어난 제품이 발생하고 있음 → 공정 평균 조정 또는 산포 감소 필요")
...
with tab2:
    ...
    kc=st.columns(4)
    grade_reason = {
        0: "들쭉날쭉이 커져도 여유 충분 - 관리 간소화 검토 가능",
        1: "아주 이상적인 수준 - 현 상태 유지",
        2: "공정 관리 강화 필요 - 1.00에 가까우면 불량 발생 가능",
        3: "불량품 발생 중 - 전체 선별 및 공정 개선 시급",
        4: "긴급 대책 필요 - 규격 재검토도 고려",
    }
    for col_,(lbl,val,sub) in zip(kc,[("Cp",Cp,"단기 산포"),("Cpk",Cpk,"단기 종합"),
        ...
        with col_:
            st.markdown(f"""<div class="kpi-card {cls_}">
              ...
              <div style="font-size:11px;color:#666;margin-top:6px">{reason}</div>
            </div>""",unsafe_allow_html=True)
    st.markdown("---")
...
with tab3:
    try:
        ...
        if spc_ct in ("I-MR","Xbar-R","Xbar-S"):
            ...
            lc=st.columns(3)
            lc[0].metric("UCL (관리 상한)", f"{us:.4f}",
                         help=f"CL + 3σ 기준. 이 선을 넘으면 Rule 1 위반(이상 감지)")
            lc[1].metric("CL (중심선)", f"{cs:.4f}",
                         help="부분군 평균의 평균(X̄̄) 또는 전체 평균")
            lc[2].metric("LCL (관리 하한)", f"{ls:.4f}",
                         help=f"CL - 3σ 기준. 이 선 아래로 내려가면 Rule 1 위반")
...
with tab5:
    ...
    for col_,res,title in [(col_a,res_a,"시나리오 A: 평균 조정"),(col_b,res_b,"시나리오 B: 산포 감소")]:
        with col_:
            ...
            if res['delta']['Cpk']>0:
                reason = ""
                if title == "시나리오 A: 평균 조정":
                    reason = f"공정 평균을 규격 중심({(USL+LSL)/2:.2f})에 가깝게 조정했기 때문 → 중심 편차 손실 제거"
                else:
                    reason = f"σ를 {res['before']['sigma']:.4f}→{res['after']['sigma']:.4f}로 줄였기 때문 → Cp = (USL-LSL)/6σ 에서 σ 감소 = 지수 상승"
                st.markdown(f"""<div class="ibox ok">
                ✅ Cpk {res['before']['Cpk']:.4f} → {res['after']['Cpk']:.4f} (향상 <b>+{res['delta']['Cpk']:.4f}</b>)<br>
                <span style="font-size:13px;color:#555">이유: {reason}</span>
                </div>""", unsafe_allow_html=True)
            else:
                reason = ""
                if title == "시나리오 A: 평균 조정":
                    reason = f"현재 평균({res['before']['mu']:.4f})이 이미 규격 중심({(USL+LSL)/2:.2f})에 가까워 조정 효과가 없음 → 이 경우 산포 감소(시나리오 B)가 더 효과적"
                else:
                    reason = "목표 σ가 현재 σ보다 크거나 같아 개선 효과 없음 → 더 낮은 σ를 목표로 설정하세요"
                st.markdown(f"""<div class="ibox warn">
                ⚠ 개선 효과 없음 (Cpk 변화: {res['delta']['Cpk']:+.4f})<br>
                <span style="font-size:13px;color:#555">이유: {reason}</span>
                </div>""", unsafe_allow_html=True)
    st.markdown("---")
